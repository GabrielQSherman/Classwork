...
const allYearsArr = Array.from({ length: 2020 + 1 }, (a, b) => b).slice(1920).reverse(),

      allMonthsArr = ['January', 'Febuary', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
...
function createInitalElms() {
    ...
    createButton({id: 'start',   class: 'navBtns', text: 'Select A Date', onClickFunc: startSelection})

    let yearSelect = createSelectElement({id: 'selectYear', class: 'selects', defaultText: 'Select A Year', array: allYearsArr});

    let monthSelect = createSelectElement({id: 'selectMonth', class: 'selects', defaultText: 'Select A Month', array: allMonthsArr});
    ...
    yearSelect.onchange = selectYear;

    monthSelect.style.display = 'none';
    monthSelect.onchange = selectMonth;
...
function selectMonth() {
    ...
    dateInfo.month = allMonthsArr.indexOf(this.value);

    console.log(dateInfo.month);

    this.style.display = 'none';

    const daysInMonthSelected = dateInfo.daysInMonths[dateInfo.month] + 1;

    let daySelect = createSelectElement({id: 'selectDay', class: 'selects', defaultText: 'Select A Day', array: Array.from({ length: daysInMonthSelected }, (a, b) => b).slice(1)});
