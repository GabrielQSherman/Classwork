    ...
    displayAvailable() {
        ...
        movieRentalDB.allMovies.filter( movieObj => movieObj.available === true).forEach( avlMovie => {
            ...
            let movieDiv = createDivElm(false, 'movieDiv'),
                ...
                movieImg = createImgElm({id: avlMovie.title, src: avlMovie.img, alt: avlMovie.title + ' Image'}),
...
function createImgElm(imgObj) {

    let image = document.createElement('img');

    image.src = imgObj.src;

    image.alt = imgObj.alt == false ? 'no image' : imgObj.alt;
...
availableDiv.style.flexWrap = 'wrap';
