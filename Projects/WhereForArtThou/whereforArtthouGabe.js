...
function whatIsInAName(collection, source) {
        ...
        //This part goes through every object in our array (collection) that we will be searching in
        ...
        for (let j = 0; j < sourceArrayKeys.length; j++) {
            ...
            //these will be changing variables for each key/value pair that we want to make sure the objects in our collection has
            let key = sourceArrayKeys[j], value = sourceArrayValues[j];

            // console.log(key, value, collection[i][key]); //collection[i][key] will return the value if it exist
