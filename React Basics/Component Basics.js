...
//more complex example of ternary conditional rendering
...
//how to use Array.map to render elements dynamicaly from an array
//map returns a value after a function fires for each element in an array
...
class MyToDoList extends React.Component {
  ...
  render() {
    const items = this.state.toDoList.map( e => <li>{e}</li>)
    return (
      <div>
        <textarea
          onChange={this.handleChange}
...
//set unique keys for each element created with the map method
...
function Frameworks() {
  const renderFrameworks = frontEndFrameworks.map(e => <li key={e+1}>{e}</li>)
  return (
    <div>
...
//using map and filter together to render elements from an array if a chosen proerty passes a conditional
class MyComponent extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  render() {
    ...
    const renderOnline = usersOnline.map( e => <li key={e.username}>{e.username}</li>)
    return (
       <div>
